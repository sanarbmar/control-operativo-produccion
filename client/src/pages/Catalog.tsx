...
type CatalogItem = { id: number; name: string; area: Area; unit: string; usesQuantity: boolean; hasVariants: boolean; defaultTargetQuantity: string | null; defaultTargetMinutes: number | null; isActive: boolean; variants: Variant[] };
...
export default function Catalog() {
  ...
  const [defaultTargetQuantity, setDefaultTargetQuantity] = useState("");
  ...
  function openForm(item?: CatalogItem) {
    setEditing(item ?? null); setName(item?.name ?? ""); setArea(item?.area ?? "produccion"); setUnit(item?.unit ?? "unidades"); setUsesQuantity(item?.usesQuantity ?? true); setHasVariants(item?.hasVariants ?? false); setDefaultTargetQuantity(item?.defaultTargetQuantity ?? ""); setDefaultTargetMinutes(item?.defaultTargetMinutes ? String(item.defaultTargetMinutes) : ""); setDialogOpen(true);
  }
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      const input = { name, area, unit, usesQuantity, hasVariants, defaultTargetQuantity: defaultTargetQuantity === "" ? null : Number(defaultTargetQuantity), defaultTargetMinutes: defaultTargetMinutes === "" ? null : Number(defaultTargetMinutes) };
      ...
      <div className="mx-auto max-w-[1200px] space-y-7">
        ...
        {query.isLoading ? <LoadingState label="Consultando catálogo…" /> : query.isError ? <ErrorState /> : items.length ? (
          <div className="space-y-4">{items.map(item => (
            <Card key={item.id} className={`border-0 shadow-[0_12px_35px_rgba(36,62,54,0.06)] ${!item.isActive ? "opacity-55" : ""}`}><CardContent className="p-5"><div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-4"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-secondary text-secondary-foreground"><Boxes className="h-5 w-5" /></div><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-extrabold">{item.name}</h3><AreaBadge area={item.area} /><Badge variant="secondary">{item.unit}</Badge>{item.defaultTargetQuantity && item.defaultTargetMinutes ? <Badge className="border-emerald-200 bg-emerald-50 text-emerald-800" variant="outline">Meta: {Number(item.defaultTargetQuantity).toLocaleString("es-CO")} en {item.defaultTargetMinutes} min</Badge> : <Badge variant="outline" className="text-muted-foreground">Sin meta</Badge>}</div><p className="mt-2 text-xs text-muted-foreground">{item.usesQuantity ? "Registra cantidades" : "Sin cantidad"}{item.hasVariants ? " · Admite variantes" : ""}</p>{item.hasVariants ? <div className="mt-3 flex flex-wrap gap-2">{item.variants.map(variant => <button key={variant.id} onClick={() => openVariant(item.id, variant)} className={`rounded-full border px-3 py-1 text-xs font-bold ${variant.isActive ? "bg-emerald-50 text-emerald-700" : "bg-muted text-muted-foreground line-through"}`}>{variant.name}</button>)}<Button variant="ghost" size="sm" onClick={() => openVariant(item.id)}><Plus className="h-3.5 w-3.5" />Variante</Button></div> : null}</div></div><div className="flex items-center justify-between gap-3 lg:justify-end"><div className="flex items-center gap-2"><span className="text-xs font-bold text-muted-foreground">{item.isActive ? "Activo" : "Inactivo"}</span><Switch checked={item.isActive} onCheckedChange={() => toggleItem(item)} /></div><Button variant="outline" size="sm" onClick={() => openForm(item)}><Pencil className="h-4 w-4" />Editar</Button></div></div></CardContent></Card>
          ))}</div>
